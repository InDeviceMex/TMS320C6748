/*
 * IMAGEPROC__en16bConectivity4.c
 *
...
void vLabelComponent4(CONECTIVITY4_TypeDef sConectivity,LCDC_AREA_TypeDef* sArea)
{
    ...
    if (*((uint16_t* )sConectivity.input+u32IndexSource) == 0) RETURN4C;   /* This pixel is not part of a component */
    if (*((uint16_t* )sConectivity.output+u32IndexDest) != 0) RETURN4C;   /* This pixel has already been labelled  */
    if(XDEST<sArea->Xmin)
      sArea->Xmin=XDEST;
    if(XDEST>sArea->Xmax)
      sArea->Xmax=XDEST;
...
int32_t IMAGEPROC__en16bConectivity4(LCDC_TFT_TypeDef *psLayerSource,LCDC_TFT_TypeDef *psLayerDest, LCDC_DIMENSIONS_TypeDef sDim, LCDC_AREA_TypeDef* sArea, uint32_t* pu32MaxArea)
{
    ...
    CONECTIVITY4_TypeDef sConectivity;
    ...
    for ( s32Height = 0; s32Height < u16DimHeight; s32Height++)
    {
        ...
        #pragma MUST_ITERATE (OPT,,OPT)
          for ( s32Width = 0; s32Width < u16DimWidth; s32Width++)
          {
              ...
              vLabelComponent4(sConectivity,psActualArea);
              psActualArea++;
