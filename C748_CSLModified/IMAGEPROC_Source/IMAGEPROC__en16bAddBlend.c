...
IMAGPROC_nStatus IMAGEPROC__en16bAddBlend(LCDC_TFT_TypeDef* psLayerSource1,LCDC_TFT_TypeDef* psLayerSource2, LCDC_TFT_TypeDef* psLayerDest, LCDC_DIMENSIONS_TypeDef sDim, float fBlend)
{
    ...
    for(s32Index=0;s32Index<u16DimHeight*u16DimWidth;s32Index++)
    {
        ...
        fAux = _rsqrsp((float)fAux1);
        fAux = fAux * (1.5f - ((float)fAux1 * fAux * fAux * 0.5f));
        fAux = fAux * (1.5f - ((float)fAux1 * fAux * fAux * 0.5f));
        fResult = (float)fAux1 * fAux;
...
/*
    ...
    for(s32Index=0;s32Index<(u16DimHeight*u16DimWidth)+u8Mod;s32Index++)
    {
        ...
        fAux1=(float)u8Value1;
        fAux2=(float)u8Value2;
        fAux1*=fAux1;
        fAux2*=fAux2;
        fAux1*=fBlend;
        fAux2*=fBlend1;
        ...
        *((uint8_t*)pu16LayerDest)= u8Result;
        pu16LayerDest++;
    ...
    for(s32Index=0;s32Index<u16DimHeight*u16DimWidth;s32Index++)
    {
        ...
        fAux1=(float)(u8Value1>>3);
        fAux2=(float)(u8Value2>>3);
        fAux1*=fAux1;
        fAux2*=fAux2;
        fAux1*=fBlend;
        fAux2*=fBlend1;
        ...
        u8Result=(uint8_t)fResult;
        u8Result<<=3;
        *((uint8_t*)pu16LayerDest+1)= u8Result;
        pu16LayerDest++;
    }
    ...
    for(s32Index=0;s32Index<u16DimHeight*u16DimWidth;s32Index++)
    {
        ...
        fAux1=(float)(u16Value1>>5);
        fAux2=(float)(u16Value2>>5);
        fAux1*=fAux1;
        fAux2*=fAux2;
        fAux1*=fBlend;
        fAux2*=fBlend1;
