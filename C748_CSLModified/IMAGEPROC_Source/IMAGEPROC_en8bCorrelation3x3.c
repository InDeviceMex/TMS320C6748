...
__inline void CORRELATION_vCompute(uint8_t* restrict pu8SubImage, uint8_t  u8Pixel[9],int8_t  s8Kernel[9],uint16_t u16Div);
__inline void CORRELATION_vFillPixelArray(uint8_t u8Pixel[9],uint8_t* restrict pu8LayerSource, uint16_t u16DimWidth );
...
IMAGPROC_nStatus IMAGEPROC__en8bCorrelation3x3(LCDC_TFT_TypeDef *psLayerSource, LCDC_TFT_TypeDef*psLayerDest, int8_t ps8Kernel[9],LCDC_DIMENSIONS_TypeDef sDim)
{
    ...
    if(u16DimWidth<=3) //size kernel
        return IMAGPROC_enERROR;

    if(u16DimHeight<=3) //size kernel
        return IMAGPROC_enERROR;
    ...
    for(u16Count=0; u16Count<9; u16Count++)
    {
